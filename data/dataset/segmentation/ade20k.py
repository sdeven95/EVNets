...
class ADE20KDataset(BaseImageDataset):
	...
	def __init__(
		...
		image_dir = os.path.join(self.root, "images", "training" if is_training else "validation")
		annotation_dir = os.path.join(self.root, "annotations", "training" if is_training else "validation")

		self.images = []
		self.masks = []
		for file_name in os.listdir(image_dir):
			if file_name.endswith(".jpg"):
				image_file_name = f"{image_dir}/{file_name}"
				mask_file_name = f"{annotation_dir}/{file_name.replace('.jpg', '.png')}"

				if os.path.isfile(image_file_name) and os.path.isfile(mask_file_name):
					self.images.append(image_file_name)
					self.masks.append(mask_file_name)

		self.ignore_label = 255
		self.bgrnd_idx = 0

		self.n_records = len(self.images)
		Opt.set_by_cfg_path(Entry.SegmentationModel, "n_classes", len(self.class_names()) - 1)  # ignore background
		...
		for aug in ("train", "val"):
			Opt.set(f"{Entry.Collate}.{aug}", "default_collate_fn")
			Opt.set(f"{Entry.Collate}.eval", None)
	...
	def __getitem__(self, batch_indexes_tup: Tuple[int, int, int]) -> Dict:
		...
		data = {"image": img}
		if not self.is_evaluation:  # for training
			data["mask"] = mask
