...
class COCODataset(BaseImageDataset):
	...
	def __init__(
		...
		ann_file = os.path.join(
			self.root, f"annotations/instances_{split}{year}.json"
		)
		self.img_dir = os.path.join(self.root, f"images/{split}{year}")
		self.split = split
	...
	def __getitem__(self, batch_indexes_tup):
		...
		if self.is_training:
			_transform = self._training_transforms(size=crop_size, ignore_idx=self.ignore_label)
		...
		im_height, im_width = rgb_img.shape[:2]  # HxWxC
		...
		data["label"] = data["mask"]  # mask is label
		del data["mask"]
